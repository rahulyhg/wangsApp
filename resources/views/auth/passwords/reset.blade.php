      ...
      <div class="card">
        <div class="card-content">
          ...
          <form class="" action="{{route('login')}}" method="POST" role="form">
            {{csrf_field()}}
            <input type="hidden" name="token" value="{{ $token }}"> <!--Autorization-->
            <div class="field">
              <label for="email" class="label">Email address</label>
              <p class="control">
                <input class="input {{$errors->has('email') ? 'is-danger' : ''}}"  type="text" name="email" placeholder="name@example.com" id="email" value="{{old('email')}}" required>
              </p>
              @if($errors->has('email'))
        ...
        <h5 class="has-text-centered m-t-20"><a href="{{route('login')}}" class="is-muted">Already Account ?</a></h5>
    </div>
  </div>
@endsection
